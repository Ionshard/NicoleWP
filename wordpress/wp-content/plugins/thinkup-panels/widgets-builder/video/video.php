...
if( ! class_exists( 'thinkup_builder_video' ) ) {

	class thinkup_builder_video extends WP_Widget {

		/* Register widget description. */
		function thinkup_builder_video() {
			$widget_ops = array('classname' => 'thinkup_builder_video', 'description' => 'Add a video from YouTube or Vimeo.' );
			parent::__construct('thinkup_builder_video', 'Video', $widget_ops);
		}
		...
		function form($instance) {
			...
			echo '<p><label for="' . $this->get_field_id('video') . '" style="display: inline-block;width: 150px;" >Video Link:</label><input class="widefat" id="' . $this->get_field_id('video') . '" name="' . $this->get_field_name('video') . '" type="text" value="' . esc_url($video) . '" style="display: inline-block;width: 200px;margin: 0;" /></p>';
		...
		function widget($args, $instance) {
			...
			if ( ! empty( $video ) ) {
				if ( strpos( $video, 'https://' ) !== false ) {
					$video = 'https://' . str_replace( 'https://', '', $video );
				} else {
					$video = 'http://' . str_replace( 'http://', '', $video );
				}
			}

			// Output video with animation if required
			if ( empty( $animate ) or $animate == 'none' ) {
				echo $wp_embed->run_shortcode('[embed]' .$video . '[/embed]');
			} else  {
				echo '<div class="animated start-' . $animate . '" title="' . $delay . '">',
					 $wp_embed->run_shortcode('[embed]' .$video . '[/embed]'),
	...
	add_action( 'widgets_init', create_function('', 'return register_widget("thinkup_builder_video");') );
}
