...
if( ! class_exists( 'thinkup_builder_progress' ) ) {
	...
	class thinkup_builder_progress extends WP_Widget {
		...
		function thinkup_builder_progress() {
		...
		function widget($args, $instance) {
			...
			if ( empty( $type ) or $type == '1' ) {
				...
				$type = 'progress progress-striped';
			} else if ($type == '3' ) {
				$type = 'progress progress-striped active';
			}

			if ( empty( $style ) or $style == '1' ) {
				$style = '';
			} else if ( $style == '2' ) {
				$style = ' bar-info';
			} else if ($style == '3' ) {
			...
			if ( ! empty ( $heading ) ) {	
				$heading = '<span class="bar-title">' . $heading . '</span>';
			}
			...
			if ( empty( $heading ) and empty( $show ) ) {
				$block = ' display: block;';
			}
			...
			echo '<div class="' . $type . '">',
				 '<div class="bar' . $style . '" style="' . $input_style . $block . '"' . $input_width . $input_delay . '>' . $heading . $show . '</div>',
				 '</div>';
	...
	add_action( 'widgets_init', create_function('', 'return register_widget("thinkup_builder_progress");') );
}
