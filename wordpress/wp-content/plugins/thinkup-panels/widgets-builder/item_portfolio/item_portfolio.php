...
if( ! class_exists( 'thinkup_builder_itemportfolio' ) ) {

	class thinkup_builder_itemportfolio extends WP_Widget {

		/* Register widget description. */
		function thinkup_builder_itemportfolio() {
			$widget_ops = array('classname' => 'thinkup_builder_itemportfolio', 'description' => 'Add a portfolio item to your content.' );
			parent::__construct('thinkup_builder_itemportfolio', 'Portfolio (Item)', $widget_ops);
		}
		...
		function form($instance) {
			...
			$toggle_style   = $instance['toggle_style'];
			...
			echo '<p><label for="' . $this->get_field_id('toggle_style') . '" style="display: inline-block;width: 150px;" >Toggle Style:</label>
				<select name="' . $this->get_field_name('toggle_style') . '" id="' . $this->get_field_id('toggle_style') . '" style="display: inline-block;width: 200px;margin: 0;" >
				<option '; ?><?php if($toggle_style == "style1") { echo "selected"; } ?><?php echo ' value="style1">Style 1</option>
				<option '; ?><?php if($toggle_style == "style2") { echo "selected"; } ?><?php echo ' value="style2">Style 2</option>
				</select>
			</p>';
		...
		function update($new_instance, $old_instance) {
			...
			$instance['toggle_style']   = $new_instance['toggle_style'];
		...
		function widget($args, $instance) {
			...
			echo do_shortcode( '[item_portfolio id="' . $post_id . '" title="' . $toggle_title . '" tags="' . $toggle_details . '" style="' . $toggle_style . '" size="' . $size . '" link_icon="' . $link_icon . '" lightbox_icon="' . $lightbox_icon . '"]' );
	...
	add_action( 'widgets_init', create_function('', 'return register_widget("thinkup_builder_itemportfolio");') );
}
