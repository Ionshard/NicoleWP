<?php
/*
Plugin Name: Page Builder by SiteOrigin (ThinkUpThemes compatible)
Plugin URI: http://siteorigin.com/page-builder/
Description: A drag and drop, responsive page builder with parallax functionality that simplifies building your website originally built by SiteOrigin. This version is a highly customized version of the page builder built by SiteOrigin and will only function properly with themes developed by ThinkUpThemes. All customizations have been made in compliance with the right and limitations of the GNU GPL license. The original site builder by SiteOrigin can be found at <a href="http://siteorigin.com/page-builder/" target="_blank">SiteOrigin</a>. Version 1.1.0 corresponds to v1.3.10 of the SiteOrigin Page Builder.
Version: 2.4.5
Author: Greg Priday
...
define('SITEORIGIN_PANELS_VERSION', '2.4.5');
...
// Arrange Think Up Themes groups and add additional fields
include plugin_dir_path(__FILE__).'thinkup-panels.php';
...
function siteorigin_panels_get_widgets(){
	...
	foreach($wp_widget_factory->widgets as $class => $widget_obj) {
		$widgets[$class] = array(
			'class' => $class,
			'id'    => $class, // ThinkUpThemes customization - allows id to be added to <li> items in widets list.
...
function siteorigin_panels_generate_css($post_id, $panels_data = false){
	...
	foreach ( $panels_data['grids'] as $gi => $grid ) {
		...
		for ( $i = 0; $i < $cell_count; $i++ ) {
			...
			if ( $cell_count > 1 ) {
				$width = round( $cell['weight'] * ( 100 - ( ($cell_count - 1 ) * 2.6 ) ), 3 ) . '%'; //ThinkUpThemes customization - Cell width takes into account padding-right: 2.6%
...
//	echo '>';
	...
	foreach ( $grids as $gi => $cells ) {
		...
		$style_attributes['class']           = array( 'panel-grid' );
		...
		// if( !empty($row_style_wrapper) ) echo $row_style_wrapper; // ThinkUpThemes customization - Remove start div for .panel-row-style
		...
		// ThinkUpThemes customization - start div for panel-grid-core class
		echo '<div class="panel-grid-core">';
...
function siteorigin_panels_the_widget( $widget_info, $instance, $grid, $cell, $panel, $is_first, $is_last, $post_id = false, $style_wrapper = '' ) {
	...
	if( !empty($style_wrapper) ) {
		$style_wrapper_start = $args['before_widget'] . $style_wrapper;
		$style_wrapper_end   = '</div>' . $args['after_widget'];
	} else {
		$style_wrapper_start = NULL;
		$style_wrapper_end   = NULL;		
	}

	if ( !empty($the_widget) && is_a($the_widget, 'WP_Widget')  ) {
		echo $style_wrapper_start;
		$the_widget->widget($args , $instance );
		echo $style_wrapper_end;
	}
	else {
		...
		if( !empty($widgets[$widget]) && !empty( $widgets[$widget]['plugin'] ) ) {
			...
			$form =
				...
				preg_replace(
					...
					sprintf(
						__('The widget 1{%1$s} is not available. Please try locate and install the missing plugin.', 'siteorigin-panels'),
						esc_html($widget)
					)
				).
				'</p></div>';
...
// Migrate all settings from ThinkUpPanels earlier than v2.0.0
register_activation_hook( __FILE__, 'thinkup_migrate_settings_pre_v2' );
function thinkup_migrate_settings_pre_v2() {
	// only in admin
	if (is_admin()) {

		// check if we already migrated
		global $wpdb;
		// get all the meta with key panels_data
		$panels_data = $wpdb->get_results($wpdb->prepare(
			"
                SELECT      *
                FROM        $wpdb->postmeta
                WHERE       meta_key = %s
		...
		// scrub through these
		foreach ($panels_data as $item) {
			$post_id = $item->post_id;
			$old_grid = maybe_unserialize($item->meta_value);
			if (isset($old_grid['grids'])) {
				// setup the $new_grid array
				$new_grid = array();
				$update = false;

				// for each grid we need to manipulate the array a bit, to match the new format
				foreach ($old_grid['grids'] as $old_grid_key => $grid) {

					if (!is_array($grid['style'])) {

						$update = true;
						// for each item in the grid manipulate the data
						foreach ($grid as $key => $grid_item) {
							if ($key == 'cells') {
								$new_grid[$old_grid_key][$key] = $grid_item;
							} elseif ($key == 'style') {
								$new_grid[$old_grid_key]['style']['class'] = $grid_item;
							} else {
								$new_grid[$old_grid_key]['style']['thinkup_' . $key] = $grid_item;
							}
						}
					}
				}
				if ($update) {
					$old_grid['grids'] = $new_grid;
					update_post_meta($post_id, 'panels_data', $old_grid);
				}
			}
		}

		// set the migrated flag, so we don't do too much magic all the time :D
		update_option('thinkup_migrated', 1);
//			delete_option('thinkup_migrated');
