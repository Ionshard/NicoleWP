...
// Remove start and end links from product on Shop page
	...
	if ( $thinkup_woocommerce_styleswitch == '1' ) {
		...
		if ( is_post_type_archive( 'product' ) or is_tax( 'product_cat' ) or is_tax( 'product_tag' ) ) {
			...
			if ( $thinkup_woocommerce_likes == '1' ) {
				$output .= getPostLikeLink( $post->ID );
...
global $thinkup_woocommerce_styleswitch;
	...
	if ( $thinkup_woocommerce_styleswitch == '1' ) {
		...
		if ( is_post_type_archive( 'product' ) or is_tax( 'product_cat' ) or is_tax( 'product_tag' ) ) {
			...
			if ( empty( $thinkup_woocommerce_layout ) or $thinkup_woocommerce_layout == 'option1' ) {
...
global $thinkup_woocommerce_styleswitch;
	...
	if ( $thinkup_woocommerce_styleswitch == '1' ) {
		...
		if ( is_post_type_archive( 'product' ) or is_tax( 'product_cat' ) or is_tax( 'product_tag' ) ) {
			...
			if ( ! empty( $thinkup_woocommerce_layout ) and $thinkup_woocommerce_layout !== 'option1' ) {
				if ( $thinkup_woocommerce_excerptshop == '1' ) {
					echo wpautop( get_post( $post->ID )->post_excerpt );
				}
			}
...
global $thinkup_woocommerce_styleswitch;
	...
	if ( $thinkup_woocommerce_styleswitch == '1' ) {
		...
		if ( is_post_type_archive( 'product' ) or is_tax( 'product_cat' ) or is_tax( 'product_tag' ) ) {
			...
			if ( empty( $thinkup_woocommerce_layout ) or $thinkup_woocommerce_layout == 'option1' or $thinkup_woocommerce_layout == 'option5' or $thinkup_woocommerce_layout == 'option6' ) {
				echo '</div><div class="one_half last"><div class="entry-content">';
			}
		}
	}
}
...
global $thinkup_woocommerce_styleswitch;
	...
	if ( $thinkup_woocommerce_styleswitch == '1' ) {
		...
		if ( is_post_type_archive( 'product' ) or is_tax( 'product_cat' ) or is_tax( 'product_tag' ) ) {
			...
			if ( empty( $thinkup_woocommerce_layout ) or $thinkup_woocommerce_layout == 'option1' or $thinkup_woocommerce_layout == 'option5' or $thinkup_woocommerce_layout == 'option6' ) {
				echo '</div></div><div class="clearboth">';
			}
		}
	}
}
...
function thinkup_woo_headerproduct() { 
global $thinkup_woocommerce_styleswitch;
...
add_action( 'woocommerce_before_shop_loop_item_title', 'thinkup_woo_headerproduct', 1 );
	...
	if ( $thinkup_woocommerce_styleswitch == '1' ) {
		...
		if ( is_singular( 'product' ) ) {
			...
			echo '<div class="woo-meta">';
				...
				// Output like button if set
				if ( empty( $_thinkup_meta_woocommercecontentcheckproduct ) or in_array( 'option1', $_thinkup_meta_woocommercecontentcheckproduct ) ) {
...
// Add title with link to individual product page
function thinkup_woo_titlerelated() {
global $thinkup_woocommerce_styleswitch;
...
add_action( 'woocommerce_after_shop_loop_item_title', 'thinkup_woo_titlerelated', 9 );
...
global $thinkup_woocommerce_styleswitch;
	...
	if ( $thinkup_woocommerce_styleswitch == '1' ) {
		...
		if ( is_singular( 'product' ) ) {
			...
			sprintf( '<p class="more-link style2"><a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" class="themebutton"><span class="more-icon"><i class="fa fa-shopping-cart fa-lg"></i></a></p>',
...
global $thinkup_woocommerce_styleswitch;
	...
	if ( $thinkup_woocommerce_styleswitch == '1' ) {
		...
		// Only output on individual product page
		if ( is_singular( 'product' ) ) {
			echo '</div><div class="three_fifth last">';
		}
	}
}
...
// Add tabbed sections
function thinkup_edit_account() {

	echo '<div id="edit-account" class="myaccount-tab tab-pane fade">';
		wc_get_template( 'myaccount/form-edit-account.php', array( 'user' => get_user_by( 'id', get_current_user_id() ) ) );
	echo '</div>';
}

function thinkup_input_accounttabs1() {
	echo '<div class="one_fifth">';
	echo '<div id="myaccount-tabs">';
	echo '<ul class="nav nav-tabs">';

//	if ( $downloads = WC()->customer->get_downloadable_products() ) {
		echo '<li><a href="#my-downloads" data-toggle="tab">' .  __( 'My Downloads', 'minamaze' ) . '</a></li>';
//	}

	echo '<li class="active"><a href="#my-orders" data-toggle="tab">' .  __( 'Order History', 'minamaze' ) . '</a></li>';
	echo '<li><a href="#my-address" data-toggle="tab">' .  __( 'Address Details', 'minamaze' ) . '</a></li>';
	echo '<li><a href="#edit-account" data-toggle="tab">' .  __( 'Edit Account', 'minamaze' ) . '</a></li>';
	echo '<li><a href="' . str_replace( '&amp;', '&', wp_logout_url( get_permalink( wc_get_page_id( 'myaccount' ) ) ) ) . '">' .  __( 'Logout', 'minamaze' ) . '</a></li>';
	echo '</ul>';
	echo '</div>';
}
function thinkup_input_accounttabs2() {
	echo '</div><div class="four_fifth last"><div id="myaccount-content" class="tab-content">';
}
function thinkup_input_accounttabs3() {
	echo '</div></div>';
}

add_action( 'woocommerce_after_my_account',  'thinkup_edit_account', 1 );
add_action( 'woocommerce_before_my_account', 'thinkup_input_accounttabs1', 5 );
add_action( 'woocommerce_before_my_account', 'thinkup_input_accounttabs2', 6 );
add_action( 'woocommerce_after_my_account',  'thinkup_input_accounttabs3', 5 );
