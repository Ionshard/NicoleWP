<?php
/**
 * ThinkUpThemes.com Upgrade notice - Minamaze Theme
 * 
...
function thinkup_latest_theme_version($interval) {
	...
	$minamaze_notifier_file_url = 'https://dl.dropboxusercontent.com/u/248874002/Updates/Themes/update_' . str_replace( ' pro', '', strtolower( $theme_data['Name'] ) ) . '.xml';

	// Check if Dropbox link is broken
	if( function_exists('curl_init') ) { // if cURL is available, use it...
		$dropbox_ch = curl_init($minamaze_notifier_file_url);
		...
		$dropbox_check = @file_get_contents($minamaze_notifier_file_url); // ...if not, use the common file_get_contents()
	}
	...
	if( empty( $dropbox_check ) ) {
		...
		$minamaze_notifier_file_url = $thinkup_cloud_prefix . '://thinkupcloud.com/cloudfiles/thinkupthemes/updates/themes/update_' . str_replace( ' pro', '', strtolower( $theme_data['Name'] ) ) . '.xml';
	}

	$minamaze_db_cache_field = 'minamaze_contempo-notifier-cache';
	$minamaze_db_cache_field_last_updated = 'minamaze_contempo-notifier-last-updated';
	$minamaze_last = get_option( $minamaze_db_cache_field_last_updated );
	$minamaze_now = time();
	// check the cache
	if ( !$minamaze_last || (( $minamaze_now - $minamaze_last ) > $interval) ) {
		// cache doesn't exist, or is old, so refresh it
		if( function_exists('curl_init') ) { // if cURL is available, use it...
			$minamaze_ch = curl_init($minamaze_notifier_file_url);
			curl_setopt($minamaze_ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($minamaze_ch, CURLOPT_HEADER, 0);
			curl_setopt($minamaze_ch, CURLOPT_TIMEOUT, 10);
			$minamaze_cache = curl_exec($minamaze_ch);
			curl_close($minamaze_ch);
		} else {
			$minamaze_cache = @file_get_contents($minamaze_notifier_file_url); // ...if not, use the common file_get_contents()
		}
		
		if ($minamaze_cache) {			
			// we got good results
			update_option( $minamaze_db_cache_field, $minamaze_cache );
			update_option( $minamaze_db_cache_field_last_updated, time() );			
		}
		// read from the cache file
		$minamaze_notifier_data = get_option( $minamaze_db_cache_field );
	}
	else {
	...
	if ( strpos( $minamaze_notifier_data,'looking for. Check out our' ) !== false) {
	} else {
		@$minamaze_xml = simplexml_load_string($minamaze_notifier_data); 
	}
	return $minamaze_xml;
}
