...
class thinkup_widget_recentcomments extends WP_Widget {
	...
	function form($instance) {
		...
		echo '<p><label for="' . $this->get_field_id('title') . '">' . __( 'Title', 'minamaze' ) . ': <input class="widefat" id="' . $this->get_field_id('title') . '" name="' . $this->get_field_name('title') . '" type="text" value="' . esc_attr($title) . '" style="width: 80px;margin-left: 113px;" /></label></p>';

		echo '<p><label for="' . $this->get_field_id('commentcount') . '">' . __( 'Number of posts', 'minamaze' ) . ': <input class="widefat" id="' . $this->get_field_id('commentcount') . '" name="' . $this->get_field_name('commentcount') . '" type="text" value="' . $commentcount . '" style="width: 80px;margin-left: 45px;" /></label></p>';
	
		echo '<p><label for="' . $this->get_field_id('excerptlength') . '">' . __( 'Excerpt Length (letters)', 'minamaze' ) . '?<input class="widefat" id="' . $this->get_field_id('excerptlength') . '" name="' . $this->get_field_name('excerptlength') . '" type="text" value="' . $excerptlength . '" style="width: 80px;margin-left: 10px;" /></label></p>';	

		echo '<p><label for="' . $this->get_field_id('commentdate') . '">' . __( 'Show comment date', 'minamaze' ) . '?</label>&nbsp;<input id="' . $this->get_field_id('commentdate') . '" name="' . $this->get_field_name('commentdate') . '" type="checkbox" ' . $commentdate_check . ' style="margin-left: 86px;" /></p>';
	}
	...
	function widget($args, $instance) {
		...
		echo $before_widget;
		$title = empty($instance['title']) ? __( 'Recent Comments', 'minamaze' ) : apply_filters('widget_title', $instance['title']);
		if (!empty($title))
