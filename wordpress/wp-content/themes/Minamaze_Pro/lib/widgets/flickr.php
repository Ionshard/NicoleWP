...
class thinkup_widget_flickr extends WP_Widget {
	...
	function form($instance) {
		...
		echo '<p><label for="' . $this->get_field_id('title') . '">' . __( 'Title', 'minamaze' ) . ': <input class="widefat" id="' . $this->get_field_id('title') . '" name="' . $this->get_field_name('title') . '" type="text" value="' . esc_attr($title) . '" /></label></p>';

		echo '<p><label for="' . $this->get_field_id('flickrid') . '">' . __( 'Flickr ID: (Find Id at <a href="//idgettr.com/">idGettr</a>)', 'minamaze' ) . '<input class="widefat" id="' . $this->get_field_id('flickrid') . '" name="' . $this->get_field_name('flickrid') . '" type="text" value="' . esc_attr($flickrid) . '" /></label></p>';

		echo '<p><label for="' . $this->get_field_id('photocount') . '">' . __( 'Number of photos', 'minamaze' ) . ': <input class="widefat" id="' . $this->get_field_id('photocount') . '" name="' . $this->get_field_name('photocount') . '" type="text" value="' . esc_attr($photocount) . '" /></label></p>';

		echo '<p><label for="' . $this->get_field_id('layout') . '" >' . __( 'Layout', 'minamaze' ) . ':
	...
	function widget($args, $instance) {
		...
		echo $before_widget;
		$title = empty($instance['title']) ? __( 'Flickr Feed', 'minamaze') : apply_filters('widget_title', $instance['title']);
		if (!empty($title))
		...
		if($flickrphoto->photos->photo) {
			...
			echo '<div class="error-icon">' . __( 'Oops! There&#39;s been an error!', 'minamaze') . '<br />' .  __( 'Check that the Flickr ID is correct at', 'minamaze') . '<a href="http://idgettr.com/">http://idgettr.com/</a>.</div>';
		}
