...
class thinkup_widget_categories extends WP_Widget {
	...
	function form($instance) {
		...
		echo '<p><label for="' . $this->get_field_id('title') . '">' . __( 'Title', 'minamaze' ) . ': <input class="widefat" id="' . $this->get_field_id('title') . '" name="' . $this->get_field_name('title') . '" type="text" value="' . esc_attr($title) . '" style="width: 95px;margin-left: 98px;" /></label></p>';
				
		echo '<p><label for="' . $this->get_field_id('postswitch') . '">' . __( 'Show post count', 'minamaze' ) . ':</label>&nbsp;<input id="' . $this->get_field_id('postswitch') . '" name="' . $this->get_field_name('postswitch') . '" type="checkbox" '; ?><?php if($postswitch == "on") { echo 'checked=checked'; } ?><?php echo ' style="margin-left: 110px;" /></p>';

		echo '<p><label for="' . $this->get_field_id('displayswitch') . '" >' . __( 'Display type', 'minamaze' ) . ':
		...
		echo '<p><label for="' . $this->get_field_id('orderswitch') . '">' . __( 'Order by', 'minamaze' ) . ': 
		...
		echo '<p><label for="' . $this->get_field_id('directionswitch') . '">' . __( 'Order direction', 'minamaze' ) . ': 
		...
		echo '<p><label for="' . $this->get_field_id('exclude') . '">' . __( 'Exclude categories', 'minamaze' ) . ': <input class="widefat" id="' . $this->get_field_id('exclude') . '" name="' . $this->get_field_name('exclude') . '" type="text" value="' . esc_attr($exclude) . '" style="width: 95px;margin-left: 20px;" /></label></p>';
	}
	...
	function widget($args, $instance) {
		...
		echo $before_widget;
		$title = empty($instance['title']) ? __( 'Categories', 'minamaze' ) : apply_filters('widget_title', $instance['title']);
		if (!empty($title))
				...
				echo '<ul>';
					foreach( $terms as $term ) {
					  echo '<li class="cat-item cat-item-24">',
							'<a href="' . get_category_link( $term->term_id ) . '" title="' . sprintf( __( 'View all posts in %s', 'minamaze' ), $term->name ) . '" ' . '><span>' . $term->name;
								if ($instance['postswitch'] == 'on') {
