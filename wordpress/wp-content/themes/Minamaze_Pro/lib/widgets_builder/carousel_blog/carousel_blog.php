...
class thinkup_builder_carouselblogharest extends WP_Widget {

	/* Register widget description. */
	function thinkup_builder_carouselblogharest() {
		$widget_ops = array('classname' => 'thinkup_builder_carouselblogharest', 'description' => 'Add a post carousel to your content.' );
		parent::__construct('thinkup_builder_carouselblogharest', 'Blog (Carousel)', $widget_ops);
	}
	...
	function form($instance) {
		...
		echo '<p><label for="' . $this->get_field_id('toggle_details') . '" style="display: inline-block;width: 150px;" >Toggle Post Meta:</label>
	...
	function widget($args, $instance) {
		...
		echo do_shortcode( '[carousel_blog items="' . $items .'" scroll="' . $scroll . '" speed="' . $speed . '" effect="' . $effect . '" title="' . $toggle_title . '" details="' . $toggle_details . '" category="' . $category . '"]' );

	}
}
add_action( 'widgets_init', create_function('', 'return register_widget("thinkup_builder_carouselblogharest");') );
