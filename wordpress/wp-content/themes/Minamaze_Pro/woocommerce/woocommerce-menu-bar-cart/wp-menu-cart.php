...
class thinkup_woo_WpMenuCart {
	...
	public function add_itemcart_to_menu( $items ) {
		...
		$menu_item_li = '<li id="woo-cart-menu">' . $wpmenucart_menu_item . $wpmenucart_menu_drop . '</li>';
	...
	public function wpmenucart_menu_item() {
		...
		$cart_contents = sprintf(_n('Cart (%d Item)', 'Cart (%d Items)', $item_data['cart_contents_count'], 'minamaze'), $item_data['cart_contents_count']);
		...
		$menu_item_a_content = '';	
		$menu_item_a_content .= '<span class="woo-cart-menu-content">'.$cart_contents.'</span>';
	...
	public function wpmenucart_menu_drop() {
		...
		if ($item_data['cart_contents_count'] == 0) {
			$menu_drop_content = '';
			$menu_drop_content = '<div class="woo-cart-menu-dropdown" style="display: none;">Cart is empty - Cart Total <span class="amount">'.$item_data['cart_total'].'</span></div>';

			$menu_item .= $menu_drop_content;
		} else {
			$menu_drop_content = '';
			$menu_drop_content .= '<div class="woo-cart-menu-dropdown" style="display: none;">Cart is not empty - Cart Total <span class="amount">'.$item_data['cart_total'].'</span></div>';
